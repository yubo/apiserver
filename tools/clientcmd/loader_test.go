...
users:
  production:
	...
	_, err := clientcmdapi.Codec.Decode([]byte(fakeKubeconfigData), &config)
	if err != nil {
...
func Example_noMergingOnExplicitPaths() {
	...
	json, err := runtime.Encode(clientcmdapi.Codec, mergedConfig)
	if err != nil {
...
func Example_mergingSomeWithConflict() {
	...
	loadingRules := ClientConfigLoadingRules{
		Precedence: []string{envVarFile.Name(), commandLineFile.Name()},
	}
