...
func getSpamKey(event *api.Event) string {
	return strings.Join([]string{
		event.Source.Component,
		event.Source.Host,
		event.InvolvedObject.Kind,
...
func (f *EventSourceObjectSpamFilter) Filter(event *api.Event) bool {
...
func (e *eventLogger) eventObserve(newEvent *api.Event, key string) (*api.Event, []byte, error) {
	var (
		patch []byte
		err   error
	)
	...
	if lastObservation.count > 0 {
		...
		//eventCopy2 := *event
		//eventCopy2.Count = 0
		//eventCopy2.LastTimestamp = api.NewTime(time.Unix(0, 0))
		//eventCopy2.Message = ""
