...
func recordToSink(sink EventSink, event *api.Event, eventCorrelator *EventCorrelator, sleepDuration time.Duration) {
...
// The return value can be ignored or used to stop recording, if desired.
func (e *eventBroadcasterImpl) StartEventWatcher(eventHandler func(*api.Event)) watch.Interface {
	watcher := e.Watch()
	go func() {
...
func (e *eventBroadcasterImpl) NewRecorder(source api.EventSource) EventRecorder {
...
func (recorder *recorderImpl) makeEvent(ref *api.ObjectReference, annotations map[string]string, eventtype, reason, message string) *api.Event {
	t := api.Time{Time: recorder.clock.Now()}
