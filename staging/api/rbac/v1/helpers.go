...
type PolicyRuleBuilder struct {
	PolicyRule rbac.PolicyRule `protobuf:"bytes,1,opt,name=policyRule"`
}

func NewRule(verbs ...string) *PolicyRuleBuilder {
	return &PolicyRuleBuilder{
		PolicyRule: rbac.PolicyRule{Verbs: verbs},
	}
}
...
func (r *PolicyRuleBuilder) RuleOrDie() rbac.PolicyRule {
	ret, err := r.Rule()
	if err != nil {
		panic(err)
	}
...
func (r *PolicyRuleBuilder) Rule() (rbac.PolicyRule, error) {
	if len(r.PolicyRule.Verbs) == 0 {
		return rbac.PolicyRule{}, fmt.Errorf("verbs are required: %#v", r.PolicyRule)
	}

	switch {
	case len(r.PolicyRule.NonResourceURLs) > 0:
		if len(r.PolicyRule.Resources) != 0 || len(r.PolicyRule.ResourceNames) != 0 {
			return rbac.PolicyRule{}, fmt.Errorf("non-resource rule may not have apiGroups, resources, or resourceNames: %#v", r.PolicyRule)
		}
	case len(r.PolicyRule.Resources) > 0:
		if len(r.PolicyRule.NonResourceURLs) != 0 {
			return rbac.PolicyRule{}, fmt.Errorf("resource rule may not have nonResourceURLs: %#v", r.PolicyRule)
		}
		//if len(r.PolicyRule.APIGroups) == 0 {
		//	// this a common bug
		//	return rbac.PolicyRule{}, fmt.Errorf("resource rule must have apiGroups: %#v", r.PolicyRule)
		//}
	default:
		return rbac.PolicyRule{}, fmt.Errorf("a rule must have either nonResourceURLs or resources: %#v", r.PolicyRule)
	}
...
type ClusterRoleBindingBuilder struct {
	ClusterRoleBinding rbac.ClusterRoleBinding `protobuf:"bytes,1,opt,name=clusterRoleBinding"`
}
...
func (r *ClusterRoleBindingBuilder) Groups(groups ...string) *ClusterRoleBindingBuilder {
	for _, group := range groups {
		r.ClusterRoleBinding.Subjects = append(r.ClusterRoleBinding.Subjects, rbac.Subject{Kind: rbac.GroupKind, Name: group})
	}
	return r
}
...
func (r *ClusterRoleBindingBuilder) Users(users ...string) *ClusterRoleBindingBuilder {
	for _, user := range users {
		r.ClusterRoleBinding.Subjects = append(r.ClusterRoleBinding.Subjects, rbac.Subject{Kind: rbac.UserKind, Name: user})
	}
	return r
}
...
func (r *ClusterRoleBindingBuilder) SAs(namespace string, serviceAccountNames ...string) *ClusterRoleBindingBuilder {
	for _, saName := range serviceAccountNames {
		r.ClusterRoleBinding.Subjects = append(r.ClusterRoleBinding.Subjects, rbac.Subject{Kind: rbac.ServiceAccountKind, Namespace: namespace, Name: saName})
	}
	return r
}

func (r *ClusterRoleBindingBuilder) BindingOrDie() rbac.ClusterRoleBinding {
	ret, err := r.Binding()
	if err != nil {
		panic(err)
	}
...
func (r *ClusterRoleBindingBuilder) Binding() (rbac.ClusterRoleBinding, error) {
	if len(r.ClusterRoleBinding.Subjects) == 0 {
		return rbac.ClusterRoleBinding{}, fmt.Errorf("subjects are required: %#v", r.ClusterRoleBinding)
	}
...
func (r *RoleBindingBuilder) Groups(groups ...string) *RoleBindingBuilder {
	for _, group := range groups {
		r.RoleBinding.Subjects = append(r.RoleBinding.Subjects, rbac.Subject{Kind: rbac.GroupKind, Name: group})
	}
	return r
}
...
func (r *RoleBindingBuilder) Users(users ...string) *RoleBindingBuilder {
	for _, user := range users {
		r.RoleBinding.Subjects = append(r.RoleBinding.Subjects, rbac.Subject{Kind: rbac.UserKind, Name: user})
	}
	return r
}
...
func (r *RoleBindingBuilder) SAs(namespace string, serviceAccountNames ...string) *RoleBindingBuilder {
	for _, saName := range serviceAccountNames {
		r.RoleBinding.Subjects = append(r.RoleBinding.Subjects, rbac.Subject{Kind: rbac.ServiceAccountKind, Namespace: namespace, Name: saName})
	}
	return r
}

// BindingOrDie calls the binding method and panics if there is an error.
func (r *RoleBindingBuilder) BindingOrDie() rbac.RoleBinding {
	ret, err := r.Binding()
	if err != nil {
		panic(err)
	}
...
// object.
func (r *RoleBindingBuilder) Binding() (rbac.RoleBinding, error) {
	if len(r.RoleBinding.Subjects) == 0 {
		return rbac.RoleBinding{}, fmt.Errorf("subjects are required: %#v", r.RoleBinding)
	}
