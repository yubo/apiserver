...
import (
	...
	"github.com/yubo/apiserver/plugin/authn/token/cache"
...
type V1Service interface {
	...
	Review(*authentication.TokenReview)
	HTTPStatusCode() int
}
...
func NewV1TestServer(s V1Service, cert, key, caCert []byte) (*httptest.Server, error) {
	...
	serveHTTP := func(w http.ResponseWriter, r *http.Request) {
		...
		var review authentication.TokenReview
...
func (m *mockV1Service) Review(r *authentication.TokenReview) {
	m.called++
...
func newV1TokenAuthenticator(serverURL string, clientCert, clientKey, ca []byte, cacheTime time.Duration, implicitAuds authenticator.Audiences) (authenticator.Token, error) {
	...
	c, err := tokenReviewInterfaceFromKubeconfig(p, testRetryBackoff, nil)
	if err != nil {
		return nil, err
	}
...
type recorderV1Service struct {
	lastRequest authentication.TokenReview
	response    authentication.TokenReviewStatus
}

func (rec *recorderV1Service) Review(r *authentication.TokenReview) {
	rec.lastRequest = *r
...
func TestV1WebhookTokenAuthenticator(t *testing.T) {
	...
	tests := []struct {
		...
		serverResponse        authentication.TokenReviewStatus
		...
		},
		{
			...
			serverResponse: authentication.TokenReviewStatus{
				...
				User: authentication.UserInfo{
					...
					Extra:    map[string]authentication.ExtraValue{"foo": {"bar", "baz"}},
				},
			},
			expectedAuthenticated: true,
			expectedUser: &user.DefaultInfo{
...
type authenticationV1UserInfo authentication.UserInfo
