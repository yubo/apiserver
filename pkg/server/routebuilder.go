...
func InstallApiDocs(apiPath string, container *restful.Container, infoProps spec.InfoProps, securitySchemes []*spec.SecurityScheme) error {
...
func (p *WebServiceBuilder) InstallApiDocs(apiPath string, container *restful.Container, infoProps spec.InfoProps, securitySchemes []*spec.SecurityScheme) error {
	// register scheme to openapi
	for _, v := range securitySchemes {
		if err := p.SecuritySchemeRegister(v.Name, v); err != nil {
			return err
		}
	}
...
func (p *webserviceBuilder) build() {
	...
	if p.server != nil {
		p.server.Handler.GoRestfulContainer.Add(p.ws)
	}
}
...
func (p *webserviceBuilder) registerHandle(rb *restful.RouteBuilder, wr *WsRoute) error {
	...
	rh, err := NewRouteHandle(wr.Handle, p.server.Serializer, p.RespWriter)
	if err != nil {
