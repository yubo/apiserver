...
func init() {
	if err := client.RegisterAuthProviderPlugin("openstack", newOpenstackAuthProvider); err != nil {
		klog.Fatalf("Failed to register openstack auth plugin: %s", err)
	}
}

func newOpenstackAuthProvider(_ string, _ map[string]string, _ client.AuthProviderConfigPersister) (client.AuthProvider, error) {
