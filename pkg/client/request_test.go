...
func TestRequestSetsHeaders(t *testing.T) {
	...
	_, _ = r.Watch(context.Background(), nil)
...
func TestRequestPathVariableFromObj(t *testing.T) {
	param := struct {
		Value string `param:"path"`
	}{
		Value: "hello",
	}
	r := (&Request{
		c:          &RESTClient{base: &url.URL{}},
		pathPrefix: "/test/{value}",
	}).VersionedParams(param, ParameterCodec)
	assert.Equal(t, "/test/hello", r.URL().String())
}
...
type PodLogOptions struct {
	Follow    bool   `param:"query"`
	Container string `param:"query"`
}

func TestRequestVersionedParams(t *testing.T) {
	r := (&Request{c: &RESTClient{content: ClientContentConfig{}}}).Param("foo", "a")
	if !reflect.DeepEqual(r.params, url.Values{"foo": []string{"a"}}) {
		t.Errorf("should have set a param: %#v", r)
	}
	r.VersionedParams(&PodLogOptions{Follow: true, Container: "bar"}, ParameterCodec)
...
func defaultContentConfig() ClientContentConfig {
	return ClientContentConfig{
		...
		Negotiator:  runtime.NewClientNegotiator(scheme.Codecs.WithoutConversion()),
	}
}
...
func TestResultIntoWithErrReturnsErr(t *testing.T) {
	...
	if err := res.Into(&Pod{}); err != res.err {
...
func TestResultIntoWithNoBodyReturnsErr(t *testing.T) {
	res := Result{
		body:    []byte{},
		decoder: scheme.Codecs.LegacyCodec(),
	}
	if err := res.Into(&Pod{}); err == nil || !strings.Contains(err.Error(), "0-length") {
...
func TestRequestWatch(t *testing.T) {
			...
			Request: &Request{
				c: &RESTClient{
					...
					Client: clientForFunc(func(req *http.Request) (*http.Response, error) {
						return &http.Response{
							StatusCode: http.StatusUnauthorized,
							Body: ioutil.NopCloser(bytes.NewReader([]byte(runtime.EncodeOrDie(scheme.Codecs.LegacyCodec(), &api.Status{
	...
	for _, testCase := range testCases {
		t.Run("", func(t *testing.T) {
			...
			watch, err := testCase.Request.Watch(context.Background(), &api.Status{})
			hasErr := err != nil
...
func TestDoRequestNewWay(t *testing.T) {
	...
	expectedObj := &Service{Spec: ServiceSpec{Ports: []ServicePort{{
		Protocol: "TCP",
		Port:     12345,
	}}}}
	expectedBody, _ := runtime.Encode(scheme.Codecs.LegacyCodec(), expectedObj)
	...
	err := c.Verb("POST").
		...
		Timeout(time.Second).
		Body([]byte(reqBody)).
		Do(context.Background()).Get(obj)
	if err != nil {
		t.Errorf("Unexpected error: %v %#v", err, err)
		return
	}

	assert.Equalf(t, expectedObj, obj, "obj")
	//if obj == nil {
	//	t.Error("nil obj")
...
func TestDoRequestNewWayReader(t *testing.T) {
	reqObj := &Pod{ObjectMeta: api.ObjectMeta{Name: "foo"}}
	reqBodyExpected, _ := runtime.Encode(scheme.Codecs.LegacyCodec(), reqObj)
	expectedObj := &Service{Spec: ServiceSpec{Ports: []ServicePort{{
		Protocol: "TCP",
		Port:     12345,
	}}}}
...
func TestDoRequestNewWayFile(t *testing.T) {
	reqObj := &Pod{ObjectMeta: api.ObjectMeta{Name: "foo"}}
	reqBodyExpected, err := runtime.Encode(scheme.Codecs.LegacyCodec(), reqObj)
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
	...
	err = c.Verb("POST").
		...
		Timeout(time.Second).
		Body(file.Name()).
		Do(context.Background()).WasCreated(&wasCreated).Get(obj)
	if err != nil {
		t.Errorf("Unexpected error: %v %#v", err, err)
		return
	}
...
func TestBody(t *testing.T) {
	...
	obj := &Pod{ObjectMeta: api.ObjectMeta{Name: "foo"}}
	bodyExpected, _ := runtime.Encode(scheme.Codecs.LegacyCodec(), obj)
...
func TestWatch(t *testing.T) {
	var table = []struct {
		...
		{watch.Added, &Pod{ObjectMeta: api.ObjectMeta{Name: "first"}}},
		{watch.Modified, &Pod{ObjectMeta: api.ObjectMeta{Name: "second"}}},
		{watch.Deleted, &Pod{ObjectMeta: api.ObjectMeta{Name: "last"}}},
	}
	...
	testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		encoder := restclientwatch.NewEncoder(streaming.NewEncoder(w, scheme.Codecs.LegacyCodec()), scheme.Codecs.LegacyCodec())
		for _, item := range table {
	...
	watching, err := s.Get().Prefix("path/to/watch/thing").Watch(context.Background(), &Pod{})
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	for _, item := range table {
		...
		assert.Equalf(t, item.t, got.Type, "type")
		assert.Equalf(t, item.obj, got.Object, "object")
...
func TestWatchUnknownContentType(t *testing.T) {
	...
	s := testRESTClient(t, testServer)
	_, err := s.Get().Prefix("path/to/watch/thing").Watch(context.Background(), &Pod{})
	if err == nil {
...
func testRESTClientWithConfig(t testing.TB, srv *httptest.Server, contentConfig ClientContentConfig) *RESTClient {
	...
	client, err := NewRESTClient(base, contentConfig, nil, nil)
	if err != nil {
