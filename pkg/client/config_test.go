...
func TestSetKubernetesDefaultsUserAgent(t *testing.T) {
	...
	if !strings.Contains(config.UserAgent, "apiserver-rest/") {
...
func TestBuildUserAgent(t *testing.T) {
	assert.New(t).Equal(
		"lynx/nicest (beos/itanium) apiserver-rest/baaaaaaaaad",
		buildUserAgent(
			"lynx", "nicest",
...
func TestDefaultKubernetesUserAgent(t *testing.T) {
	assert.New(t).Contains(DefaultKubernetesUserAgent(), "apiserver")
}
		...
		},
		{
			...
			c: &Config{
				...
				ExecProvider: &api.ExecConfig{
					Args:   []string{"secret"},
					Env:    []api.ExecEnvVar{{Name: "secret", Value: "s3cr3t"}},
...
func TestConfigSprint(t *testing.T) {
	c := &Config{
		...
		AuthConfigPersister: AuthProviderConfigPersister(fakeAuthProviderConfigPersister{}),
		ExecProvider: &api.ExecConfig{
			Command:            "sudo",
			Args:               []string{"secret"},
			Env:                []api.ExecEnvVar{{Name: "secret", Value: "s3cr3t"}},
	...
	want := fmt.Sprintf(
		`&client.Config{Host:"localhost:8080", APIPath:"v1", ContentConfig:client.ContentConfig{AcceptContentTypes:"application/json", ContentType:"application/json", NegotiatedSerializer:runtime.NegotiatedSerializer(nil)}, Username:"gopher", Password:"--- REDACTED ---", BearerToken:"--- REDACTED ---", BearerTokenFile:"", Impersonate:client.ImpersonationConfig{UserName:"gopher2", Groups:[]string(nil), Extra:map[string][]string(nil)}, AuthProvider:api.AuthProviderConfig{Name: "gopher", Config: map[string]string{--- REDACTED ---}}, AuthConfigPersister:rest.AuthProviderConfigPersister(--- REDACTED ---), ExecProvider:api.ExecConfig{Command: "sudo", Args: []string{"--- REDACTED ---"}, Env: []ExecEnvVar{--- REDACTED ---}, APIVersion: "", ProvideClusterInfo: true, Config: runtime.Object(--- REDACTED ---)}, TLSClientConfig:client.sanitizedTLSClientConfig{Insecure:false, ServerName:"", CertFile:"a.crt", KeyFile:"a.key", CAFile:"", CertData:[]uint8{0x2d, 0x2d, 0x2d, 0x20, 0x54, 0x52, 0x55, 0x4e, 0x43, 0x41, 0x54, 0x45, 0x44, 0x20, 0x2d, 0x2d, 0x2d}, KeyData:[]uint8{0x2d, 0x2d, 0x2d, 0x20, 0x52, 0x45, 0x44, 0x41, 0x43, 0x54, 0x45, 0x44, 0x20, 0x2d, 0x2d, 0x2d}, CAData:[]uint8(nil), NextProtos:[]string{"h2", "http/1.1"}}, UserAgent:"gobot", DisableCompression:false, Transport:(*client.fakeRoundTripper)(%p), WrapTransport:(transport.WrapperFunc)(%p), QPS:1, Burst:2, RateLimiter:(*client.fakeLimiter)(%p), WarningHandler:client.fakeWarningHandler{}, Timeout:3000000000, Dial:(func(context.Context, string, string) (net.Conn, error))(%p), Proxy:(func(*http.Request) (*url.URL, error))(%p)}`,
