...
func jsonTabArrayTitle(w io.Writer, v reflect.Value) {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		inline := false
		if len(jsonTags) > 1 {
			if util.StringArrayContains("inline", jsonTags[1:]) {
				inline = true
			}
		}
		if inline {
...
func fieldOut(v interface{}, out string) string {
	...
	case "date":
		if data, ok := v.(int64); ok {
			...
			return util.FmtTs(data)
		}
	case "oneline":
		if data, ok := v.(string); ok {
			return util.FirstLine(data)
		}
	case "substr":
		if data, ok := v.(string); ok {
			return util.SubStr2(data, util.Atoi(args[2]), util.Atoi(args[3]))
		}
	}
	return fmt.Sprintf("%v", v)
}
