...
type Server interface {
	...
	GetExec(*api.ExecRequest) (*api.ExecResponse, error)
	GetAttach(req *api.AttachRequest) (*api.AttachResponse, error)
	GetPortForward(*api.PortForwardRequest) (*api.PortForwardResponse, error)
...
type Runtime interface {
	Exec(containerID string, cmd []string, in io.Reader, out, err io.WriteCloser, tty bool, resize <-chan term.TerminalSize) error
	Attach(containerID string, in io.Reader, out, err io.WriteCloser, tty bool, resize <-chan term.TerminalSize) error
	PortForward(podSandboxID string, port int32, stream io.ReadWriteCloser) error
}

// Provider is the interface to execute the commands and provide the streams.
type Provider interface {
	remotecommandserver.Executor
	remotecommandserver.Attacher
...
func validateExecRequest(req *api.ExecRequest) error {
	if req.ContainerId == "" {
...
func (s *server) GetExec(req *api.ExecRequest) (*api.ExecResponse, error) {
...
func validateAttachRequest(req *api.AttachRequest) error {
	if req.ContainerId == "" {
		return status.Errorf(codes.InvalidArgument, "missing required container_id")
	}
	if req.Tty && req.Stderr {
...
func (s *server) GetAttach(req *api.AttachRequest) (*api.AttachResponse, error) {
	...
	return &api.AttachResponse{
		Url: s.buildURL("attach", token),
	}, nil
}

func (s *server) GetPortForward(req *api.PortForwardRequest) (*api.PortForwardResponse, error) {
	if req.PodSandboxId == "" {
...
func (s *server) serveExec(req *restful.Request, resp *restful.Response) {
	...
	exec, ok := cachedRequest.(*api.ExecRequest)
	if !ok {
		http.NotFound(resp.ResponseWriter, req.Request)
		return
	}
...
func (s *server) serveAttach(req *restful.Request, resp *restful.Response) {
	...
	attach, ok := cachedRequest.(*api.AttachRequest)
	if !ok {
		http.NotFound(resp.ResponseWriter, req.Request)
		return
	}
...
func (s *server) servePortForward(req *restful.Request, resp *restful.Response) {
	...
	pf, ok := cachedRequest.(*api.PortForwardRequest)
	if !ok {
		http.NotFound(resp.ResponseWriter, req.Request)
		return
	}
...
func NewProvider(r Runtime) Provider {
	return &criAdapter{Runtime: r}
}
...
func (a *criAdapter) ExecInContainer(podName string, podUID types.UID, container string, cmd []string, in io.Reader, out, err io.WriteCloser, tty bool, resize <-chan term.TerminalSize, timeout time.Duration) error {
	return a.Runtime.Exec(container, cmd, in, out, err, tty, resize)
}

func (a *criAdapter) AttachContainer(podName string, podUID types.UID, container string, in io.Reader, out, err io.WriteCloser, tty bool, resize <-chan term.TerminalSize) error {
