...
func (n *fakeNegotiater) EncoderForVersion(serializer runtime.Encoder) runtime.Encoder {
	return n.serializer
}

func (n *fakeNegotiater) DecoderToVersion(serializer runtime.Decoder) runtime.Decoder {
	return n.serializer
}
...
func TestNegotiate(t *testing.T) {
	...
	for i, test := range testCases {
		...
		case err != nil:
			...
			if status.Status().Status != api.StatusFailure || status.Status().Code != http.StatusNotAcceptable {
				t.Errorf("%d: failed: %v", i, err)
				continue
			}
			continue
