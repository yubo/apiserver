...
// returns an error.
func NegotiateOutputMediaType(req *http.Request, ns runtime.NegotiatedSerializer) (MediaTypeOptions, runtime.SerializerInfo, error) {
	mediaType, ok := NegotiateMediaTypeOptions(req.Header.Get("Accept"), ns.SupportedMediaTypes())
	if !ok {
...
//	AllowsMediaTypeTransform(mimeType, mimeSubType string) bool
...
func (emptyEndpointRestrictions) AllowsMediaTypeTransform(mimeType string, mimeSubType string) bool {
	//return gvk == nil
	return true
}
...
func acceptMediaTypeOptions(params map[string]string, accepts *runtime.SerializerInfo) (MediaTypeOptions, bool) {
	...
	for k, v := range params {
		...
		case "stream":
			if len(v) > 0 && accepts.StreamSerializer == nil {
...
func NegotiateMediaTypeOptions(header string, accepted []runtime.SerializerInfo) (MediaTypeOptions, bool) {
	...
	for i := range clauses {
		...
		for i := range accepted {
			...
			case clause.Type == accepts.MediaTypeType && clause.SubType == accepts.MediaTypeSubType,
				...
				if retVal, ret := acceptMediaTypeOptions(clause.Params, accepts); ret {
					return retVal, true
				}
			}
		}
